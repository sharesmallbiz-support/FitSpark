...
export async function generatePersonalizedProgram(
  ...
  const videosByType = availableVideos.reduce((acc, video) => {
    const key = video.exerciseType || "uncategorized";
    if (!acc[key]) acc[key] = [];
    acc[key].push(video);
    return acc;
...
- Health Concerns: ${
        fitnessGoals.healthConcerns.length > 0
          ? fitnessGoals.healthConcerns.join(", ")
          : "None specified"
      }
- Preferred Activities: ${
        fitnessGoals.preferredActivities.length > 0
          ? fitnessGoals.preferredActivities.join(", ")
          : "All activities welcome"
      }
`
    : "";

  const prompt = `
Create a personalized 30-day fitness program for ${name}, a ${age}-year-old person looking to ${
    fitnessGoals?.primaryGoal === "weight-loss"
      ? "lose weight"
      : fitnessGoals?.primaryGoal === "strength"
      ? "build strength"
      : fitnessGoals?.primaryGoal === "endurance"
      ? "improve endurance"
      : fitnessGoals?.primaryGoal === "flexibility"
      ? "increase flexibility"
      : "improve overall health"
  } from ${startWeight} lbs to ${targetWeight} lbs.
...
${
  fitnessGoals?.healthConcerns && fitnessGoals.healthConcerns.length > 0
    ? `\nIMPORTANT: Accommodate these health considerations: ${fitnessGoals.healthConcerns.join(
        ", "
      )}. Modify exercises accordingly for safety.`
    : ""
}
  ...
  .map(
    ([type, videos]) =>
      `${type}: ${videos
        .map(
          (v) =>
            `"${v.title}" (${v.duration}min, effort: ${v.effortLevel}/5, id: ${v.id})`
        )
        .join(", ")}`
  )
  .join("\n")}
...
1. Starts gentle and builds intensity gradually based on fitness level: ${
    fitnessGoals?.fitnessLevel || "beginner"
  }
2. Alternates between different exercise types${
    fitnessGoals?.preferredActivities &&
    fitnessGoals.preferredActivities.length > 0
      ? ` (prioritize: ${fitnessGoals.preferredActivities.join(", ")})`
      : ""
  }
...
6. Each workout should target ${
    fitnessGoals?.timeCommitment || 30
  } minutes total (adjustable based on user capacity)
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a certified fitness trainer specializing in programs for people over 55. Create safe, progressive, and engaging workout plans. Always respond with valid JSON.",
        },
        {
          role: "user",
          content: prompt,
...
`;
  ...
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a motivational fitness coach specializing in encouraging people over 55. Create inspiring, theme-appropriate messages.",
        },
        {
          role: "user",
          content: prompt,
    ...
    return (
      ...
      "Keep going! Every day makes you stronger."
    );
  } catch (error) {
...
export async function generateAchievementBadge(
  ...
  theme: Theme,
  userName: string
): Promise<{ title: string; description: string; icon: string }> {
  const prompt = `
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert at creating motivational achievement badges for fitness apps. Keep responses concise and impactful.",
        },
        {
          role: "user",
          content: prompt,
